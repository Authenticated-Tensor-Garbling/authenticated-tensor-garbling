...
impl AuthTensorGen {
    ...
    /// Generate input wire labels at garble time and emit the eval-side labels
    /// for hand-off to `AuthTensorEval::install_input_labels`.
    ///
    /// Per `.planning/LABELS-BUG-CONTEXT.md` "Correct Target Architecture"
    /// (BUG-02), input wire labels are NOT a preprocessing artifact — they are
    /// generated by the garbler at garble time, after the masked input bits
    /// `d_x = x XOR α` and `d_y = y XOR β` are revealed via an opening step
    /// between the parties.
    ///
    /// In a real two-party protocol, `d_x` / `d_y` come from a dedicated
    /// opening protocol over the α / β auth-bit shares. In this in-process
    /// simulation, the test orchestrator computes
    /// `d_x[i] = ((x >> i) & 1) ^ gb.alpha_auth_bit_shares[i].bit()
    ///                          ^ ev.alpha_auth_bit_shares[i].bit()`
    /// and the symmetric β path, then passes the result here.
    ///
    /// Generates `self.n` fresh random blocks with `lsb == 0` and stores them
    /// as the gen-side `label_0` for `x`; analogously `self.m` blocks for `y`.
    /// Returns the eval-side label vectors `label_0[i] XOR (d_x[i] · δ_a)`
    /// (and likewise for y) for the caller to hand off.
    ///
    /// MUST be called before `garble_first_half` / `garble_second_half`.
    ///
    /// # Panics
    /// Panics if `d_x.len() != self.n` or `d_y.len() != self.m`.
    pub fn prepare_input_labels<R: Rng + CryptoRng>(
        ...
        assert_eq!(
            d_x.len(), self.n,
            "prepare_input_labels: d_x.len() ({}) must equal self.n ({})",
            d_x.len(), self.n
        );
        assert_eq!(
            d_y.len(), self.m,
            "prepare_input_labels: d_y.len() ({}) must equal self.m ({})",
            d_y.len(), self.m
        );

        let delta_a_block = *self.delta_a.as_block();

        self.x_labels = Vec::with_capacity(self.n);
        let mut eval_x_labels: Vec<Block> = Vec::with_capacity(self.n);
        for i in 0..self.n {
            let mut label_0 = Block::random(rng);
            label_0.set_lsb(false);
            self.x_labels.push(label_0);
            let eval_label = if d_x[i] { label_0 ^ delta_a_block } else { label_0 };
            eval_x_labels.push(eval_label);
        }

        self.y_labels = Vec::with_capacity(self.m);
        let mut eval_y_labels: Vec<Block> = Vec::with_capacity(self.m);
        for j in 0..self.m {
            let mut label_0 = Block::random(rng);
            label_0.set_lsb(false);
            self.y_labels.push(label_0);
            let eval_label = if d_y[j] { label_0 ^ delta_a_block } else { label_0 };
            eval_y_labels.push(eval_label);
        }

        (eval_x_labels, eval_y_labels)
    }
