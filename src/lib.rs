...
/// Re-exports crate-internal helpers needed by the Criterion bench target.
///
...
pub mod bench_internals {
    pub use crate::assemble_helpers::{
        assemble_c_alpha_beta_blocks_p2, assemble_e_input_wire_blocks_p1,
    };
}
...
mod tests {
    ...
    fn test_semihonest_tensor_product() {
        ...
        let (x_dgb, y_dgb) = gb.get_first_inputs();
        let (ev_x, ev_y) = ev.get_first_inputs();

        assert!(
            verify_column_matrix_sharing(masked_x & n_bitmask, &x_dgb, &ev_x, &delta, n)
        );
        assert!(
            verify_column_matrix_sharing(clear_y & m_bitmask, &y_dgb, &ev_y, &delta, m)
        );
        ...
        assert!(
            verify_tensor_output(masked_x & n_bitmask, clear_y & m_bitmask, n, m, &gb.gb_first_half_out_dgb, &ev.ev_first_half_out_dgb, &delta)
        );


        let (x_dgb, y_dgb) = gb.get_second_inputs();
        let (ev_x, ev_y) = ev.get_second_inputs();

        assert!(
            verify_column_matrix_sharing(masked_y & m_bitmask, &x_dgb, &ev_x, &delta, m)
        );
        assert!(
            verify_column_matrix_sharing(alpha & n_bitmask, &y_dgb, &ev_y, &delta, n)
        );
        ...
        assert!(
            verify_tensor_output(masked_y & m_bitmask, alpha & n_bitmask, m, n, &gb.gb_second_half_out_dgb, &ev.ev_second_half_out_dgb, &delta)
        );
    ...
    fn test_auth_tensor_product() {
        ...
        let mut fpre = TensorFpre::new_with_delta(54, n, m, 8, delta_gb, delta_ev);
        fpre.generate_ideal();
        ...
        // `ev.ev_masked_x_bits` is the cleartext d_x vector (gb-side is 0-vec).
        // masked_x := x ⊕ α  ⇒  α = x ⊕ masked_x. Same for β/y.
        let masked_x: usize = ev.ev_masked_x_bits.iter().enumerate()
            .map(|(i, &b)| (b as usize) << i).sum();
        let masked_y: usize = ev.ev_masked_y_bits.iter().enumerate()
        ...
        assert!(
            verify_column_matrix_sharing(masked_x, &x_dgb, &ev_x, &delta_gb, n)
        );
        assert!(
            verify_column_matrix_sharing(beta, &y_dgb, &ev_y, &delta_gb, m)
        );


        let (gb_chunk_levels, gb_chunk_cts) = gb.garble_first_half();
        ev.evaluate_first_half(gb_chunk_levels, gb_chunk_cts);
        ...
        assert!(
            verify_column_matrix_sharing(masked_y, &x_dgb, &ev_x, &delta_gb, m)
        );
        assert!(
            verify_column_matrix_sharing(input_x, &y_dgb, &ev_y, &delta_gb, n)
        );


        let (gb_chunk_levels, gb_chunk_cts) = gb.garble_second_half();
        ev.evaluate_second_half(gb_chunk_levels, gb_chunk_cts);
        ...
        for i in 0..n {
            for j in 0..m {
                ...
                let gb_share = gb.correlated_dgb[j * n + i];
                let ev_share = ev.correlated_dgb[j * n + i];

                if expected_val {
                    assert_eq!(gb_share, ev_share ^ delta_gb.as_block(), "At position ({},{}): gb_out should equal ev_out ^ delta when expected_val=1", i, j);
                } else {
    ...
    fn run_full_protocol_1(backend: &dyn TensorPreprocessing, x: usize, y: usize) {
        ...
        // After encode_inputs(x, y), `ev.ev_masked_x_bits[i] = x_i ⊕ α_i` (and
        // gb-side is the 0-vec by the asymmetric sharing); with x = y = 0 in
        // these tests this equals λ_α / λ_β exactly.
        let l_alpha_pub: Vec<bool> = ev.ev_masked_x_bits.clone();
        let l_beta_pub:  Vec<bool> = ev.ev_masked_y_bits.clone();
        ...
        let ev_v_alpha_dev: Vec<Block> = (0..n)
            .map(|i| if l_alpha_pub[i] {
                ev.alpha_dev[i] ^ *ev.delta_ev.as_block()
            } else {
                ev.alpha_dev[i]
            })
            .collect();
        let gb_v_beta_dev: Vec<Block> = gb.beta_dev.clone();
        let ev_v_beta_dev: Vec<Block> = (0..m)
            .map(|j| if l_beta_pub[j] {
                ev.beta_dev[j] ^ *ev.delta_ev.as_block()
            } else {
                ev.beta_dev[j]
            })
            .collect();
        ...
        // gen_block[k] == eval_block[k] (their XOR is bit·δ_ev with bit=0).
        assert!(
    ...
    ///   - verify_tensor_output asserts post-garble_final shares reconstruct to (x⊗y)·δ_gb
    ...
    ///   - pass the D_ev consistency check (c_α, c_β == 0) under ev.delta_ev
    ///
    ...
    fn run_full_protocol_2(backend: &dyn TensorPreprocessing, x: usize, y: usize) {
        ...
        // gb_d_gb_out[j*n+i] equals gb.gb_first_half_out_dgb[(i, j)] (column-major layout)
        ...
        // ===========================================================================
        // PART B: P2 consistency check — c_α / c_β assembled under delta_ev
        ...
        // Honest CheckZero under delta_ev per 6_total.tex §222.
        assert!(
            block_check_zero(&c_gen_blocks_p2, &c_eval_blocks_p2),
            "honest Protocol 2 run must pass block_check_zero on c_α/c_β under D_ev (delta_ev)"
        );
    }
    ...
    fn test_protocol_1_e_input_wire_check_aborts_on_garbler_d_ev_tamper() {
        ...
        // by passing a tampered `gb_v_alpha_dev[0]` to the helper.
        //
        // The XOR is `gb.delta_gb.as_block()`, whose LSB is 1 — so the tamper leaks
        // into `combined_e_block.lsb()` and breaks the e_a_bit = 0 invariant,
        // causing `block_check_zero` under δ_ev to return false.
        ...
        let mut gb_v_alpha_dev: Vec<Block> = gb.alpha_dev.clone();
        let gb_v_beta_dev: Vec<Block> = gb.beta_dev.clone();

        // TAMPER: corrupt gb's [v_a D_ev]^gb at i=0 by XORing in delta_gb (LSB=1).
        // This simulates a malicious garbler whose announced share deviates from
        // what input encoding requires.
        gb_v_alpha_dev[0] ^= *gb.delta_gb.as_block();
        ...
        assert!(
            ...
            "tampered gb_v_alpha_dev must cause paper-faithful block_check_zero to abort"
        );
    }
    ...
    ///
    /// Concretely: tamper `gb_v_alpha_dev[0]` by XORing `δ_ev` (which has
    ...
    fn test_protocol_1_e_input_wire_block_check_aborts_on_lsb_zero_tamper() {
        ...
        // TAMPER: XOR δ_ev (LSB=0 by `Delta::random_b` invariant) into
        // gb_v_alpha_dev[0]. The combined block's LSB is unchanged, so the
        // prior LSB-only check would have missed this.
        debug_assert_eq!(ev.delta_ev.as_block().lsb(), false,
            "δ_ev must have LSB=0 for this test to exercise the missed-by-LSB path");
        gb_v_alpha_dev[0] ^= *ev.delta_ev.as_block();
