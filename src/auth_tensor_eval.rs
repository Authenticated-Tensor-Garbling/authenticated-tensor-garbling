...
impl AuthTensorEval {
    ...
    /// (BUG-02), input wire labels are generated by the garbler at garble time
    /// and transmitted to the evaluator as part of the online-phase message,
    /// not propagated through preprocessing. In this in-process simulation,
    /// the "transmission" is the return value of
    /// `AuthTensorGen::prepare_input_labels` passed directly here.
    ///
    /// MUST be called before `evaluate_first_half` / `evaluate_second_half`.
    ///
    /// # Panics
    /// Panics if `x_labels.len() != self.n` or `y_labels.len() != self.m`.
    pub fn install_input_labels(
        &mut self,
        x_labels: Vec<Block>,
        y_labels: Vec<Block>,
    ) {
        assert_eq!(
            x_labels.len(), self.n,
            "install_input_labels: x_labels.len() ({}) must equal self.n ({})",
            x_labels.len(), self.n
        );
        assert_eq!(
            y_labels.len(), self.m,
            "install_input_labels: y_labels.len() ({}) must equal self.m ({})",
            y_labels.len(), self.m
        );
